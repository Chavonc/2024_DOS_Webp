...
def check_user_exists(user):
    users_list = []#存儲用戶名稱
    with open("./user.txt", "r") as f: #打開user.txt進行讀取
        for line in f:
            name = line.replace("\n", "") #移除每行結尾的換行符
            users_list.append(str(name)) #將用戶名稱添加到列表中

    if user in users_list: #檢查給定的用戶名稱是否存在於列表中
        return True
    else:
        return False
